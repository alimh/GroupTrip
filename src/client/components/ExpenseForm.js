...
export const ExpenseForm = (props) => {
  ...
  const canEdit = expenseObj === null || expenseObj.canEdit;

  const label = () => {
    if (!expenseObj) return false;
    if (expenseObj.removed_at) {
      return <Badge variant="danger">Removed</Badge>;
    }
    if (expenseObj.needsAttention && canEdit) {
      return <Badge variant="warning">Incomplete</Badge>;
    }
    return <div key="blank-label" />;
  };

  const { date, paidBy = { id: '' }, note } = expenseObj || {};
  const { amount, category = { id: '' }, splitBy } = expenseObj || {};
  ...
  const showRemoveButton = expenseObj ? () => onRemove() : false;
  return (
